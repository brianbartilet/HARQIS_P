from Business.trading.models.psei import *

ROUND_PRICE = 4


def get_psei_units_available_from_price(price: float, amount_risk: float):
    lots = 0
...
def get_psei_ceiling_price(price: float):
    price_ = price + (0.5 * price)
    return round(price_, ROUND_PRICE)


def normalize_psei_price(price: float):
    for lot_sizing in PSEILots.keys():
        if PSEILots[lot_sizing][0][0] <= price <= PSEILots[lot_sizing][0][1]:
