...
class ApiServiceGoogleSheets(BaseApiServiceGoogle):
    ...
    def __init__(self, source_id, scopes_list):
        super(ApiServiceGoogleSheets, self).__init__(source_id, scopes_list=scopes_list)
        self.initialize()
        self.row_data = []
        self.data = {
            'values': self.row_data
        }
        self.sheet_id = self.parameters['sheet_id']
