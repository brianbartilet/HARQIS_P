...
def initialize_hud_configuration(source_id,
                                 hud_item_name: str,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                static_path = os.path.join(ENV_ROOT_DIRECTORY, 'Applications', 'Rainmeter', 'static')
                config_ini = ConfigHelperRainmeter()
                skin_name = apps_config[source_id]['skin_name']
                skin_path = os.path.join(ENV_ROOT_DIRECTORY,
                                         'Workflows',
                                         'hud',
                                         'skins',
                                         skin_name)
                ...
                if not os.path.exists(skin_path):
                    ...
                    dest = os.path.join(skin_path, '@Resources')
                    shutil.copytree(src, dest)

                    #  copy options
                    src = os.path.join(static_path, 'Options')
                    dest = os.path.join(skin_path, 'Options')
                    shutil.copytree(src, dest)

                ini_path = os.path.join(skin_path, hud_item_name.replace(' ', ''))
                ...
                cmd_act_cfg = '"{0}" !ActivateConfig "{1}\\{2}" "{3}"'\
                    .format(apps_config[source_id]['bin_path'], skin_name, short_hud_name, file_ini_new)
                subprocess.call(shlex.split(cmd_act_cfg))

                cmd_refresh_app = '"{0}" !RefreshApp'.format(apps_config[source_id]['bin_path'])
